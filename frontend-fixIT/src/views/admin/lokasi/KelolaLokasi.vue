<template>
  <div class="card-container">
    ...
    <div class="card-header">
      <div>
        <h3>Daftar Lokasi</h3>
        <p class="subtitle">Kelola dan atur lokasi aset atau titik kerusakan di sini.</p>
      </div>
      <div class="header-actions">
        <span class="badge-count">{{ locationCountLabel }}</span>
        <RouterLink to="/TambahLokasi" class="nav-item">
    ...
    <div class="table-responsive">
      <table class="custom-table">
        ...
        <tbody>
          ...
          <tr v-else-if="locations.length === 0">
            <td colspan="4" class="text-center py-8">
              <div class="empty-state">
                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>
          ...
          <!-- Data List -->
          <tr v-else v-for="(location, index) in locations" :key="location.id" class="table-row">
            <td class="font-medium text-muted">{{ index + 1 }}</td>
            <td class="font-semibold text-dark">{{ location.name }}</td>
            <td class="text-muted">{{ location.description || '-' }}</td>
            <td>
              <div class="action-buttons">
                <!-- Edit Button -->
                <RouterLink :to="'/edit-lokasi/' + location.id" class="btn-icon btn-edit" title="Edit Lokasi">
                ...
                >
                  <span v-if="deletingId === location.id" class="spinner-small"></span>
...
// --- Constants -------------------------------------------------------------
const API_ENDPOINT = 'http://10.10.11.145:8000/api/locations';
const MESSAGE_AUTO_HIDE_MS = 4000;

// --- Computed --------------------------------------------------------------
const locationCountLabel = computed(() => `${locations.value.length} Lokasi`);
...
const fetchLocations = async () => {
  ...
  try {
    ...
    // ApiResponse Trait dari Laravel nge-wrap data ke response.data.data
    ...
    errorMessage.value = extractErrorMessage(error, 'Gagal mengambil data lokasi.');
  } finally {
    loading.value = false;
  }
};
...
const handleDelete = async (id) => {
  ...
  try {
    ...
    successMessage.value = response.data.message || 'Lokasi berhasil dihapus.';
    locations.value = locations.value.filter((loc) => loc.id !== id);
    autoHideMessages();
  } catch (error) {
    errorMessage.value = extractErrorMessage(error, 'Tidak dapat menghapus lokasi ini.');
    autoHideMessages();
  } finally {
